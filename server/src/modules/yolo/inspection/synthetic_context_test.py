...
def _run_case(
    *,
    ...
    used_context_refinement = projection.startswith("context_feature_affine")
...
def _case_notes(
    *,
    ...
    if not weak_context and not used_context_refinement:
        notes.append("достаточно контекста, но context_feature_affine не сработал")
    if weak_context and used_context_refinement:
...
def _build_summary(results: list[SyntheticResult]) -> dict[str, Any]:
    ...
    drifts = [result.center_drift_px for result in results]
    return {
        "total": total,
        "passed": passed,
        "failed": failed,
        ...
        "shape_names": sorted({result.object_shape for result in results}),
    }
    ...
    for result in results:
        ...
        cards.append(
            ...
            <article class="case {'pass' if result.passed else 'fail'}">
              ...
              <table>
                <tr><th>shape</th><td>{html.escape(result.object_shape)}</td><th>status</th><td>{html.escape(result.status)}</td></tr>
                <tr><th>projection</th><td colspan="3">{html.escape(result.projection)}</td></tr>
